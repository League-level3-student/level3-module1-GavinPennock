...
public class LongChipCompetition {
	...
	public void run() {
		...
		for (int i = 0; i < getTheBand().size(); i++) {
			for (int j = 0; j < getTheBand().get(i).getChips().size(); j++) {
				if (getTheBand().get(i).getChips().get(j).getLength() > longestChip) {
					longestChip = getTheBand().get(i).getChips().get(j).getLength();
					longestChipOwner = getTheBand().get(i).getName();
				}
			}
		}
